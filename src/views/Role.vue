<template>
  <div class="role-container">
      ...
      <div class="dialog-content">
        <el-form :model="roleForm" :rules="rules" ref="roleFormRef" label-width="100px">
          <el-form-item label="角色编号" prop="code">
            <el-input v-model="roleForm.code" placeholder="请输入" clearable></el-input>
          </el-form-item>
          
          <el-form-item label="角色名称" prop="name">
            <el-input v-model="roleForm.name" placeholder="请输入" clearable></el-input>
          </el-form-item>
    ...
    >
      ...
      <template #footer>
        <span class="dialog-footer">
          <el-button @click="permissionDialogVisible = false" class="cancel-btn">取 消</el-button>
          <el-button type="primary" @click="savePermissions" class="confirm-btn">确 定</el-button>
        </span>
      </template>
    </el-dialog>
  </div>
...
const confirmDelete = () => {
  // 模拟删除操作
  const index = roleList.value.findIndex(item => item.id === roleToDelete.value.id)
  if (index !== -1) {
...
const handlePermission = (row) => {
  ...
  if (row.id === 1) { // 超级管理员拥有所有权限
    checkedPermissions.value = [0, 1, 11, 12, 13, 2, 21, 22, 3, 31, 32, 33, 4]
  } else if (row.id === 2) { // 普通管理员拥有部分权限
    checkedPermissions.value = [1, 11, 12, 2, 21, 22, 4]
  } else {
    // 其他角色随机分配一些权限
    checkedPermissions.value = [1, 11, 2, 21, 3, 31]
  }
}
  ...
  if (data.id === 0) {
    ...
    if (checked) {
      ...
      const selectAllNodes = (nodes) => {
        nodes.forEach(node => {
          permissionTree.value.setChecked(node.id, true, false)
          if (node.children && node.children.length > 0) {
            selectAllNodes(node.children)
          }
        })
      }
      selectAllNodes(data.children)
    } else {
      // 取消选中所有节点
      const unselectAllNodes = (nodes) => {
        nodes.forEach(node => {
          permissionTree.value.setChecked(node.id, false, false)
          if (node.children && node.children.length > 0) {
            unselectAllNodes(node.children)
          }
        })
      }
      unselectAllNodes(data.children)
    }
  } 
  // 如果是父节点，处理子节点的选中状态
    ...
    data.children.forEach(child => {
      ...
      if (child.children && child.children.length > 0) {
        child.children.forEach(grandChild => {
          permissionTree.value.setChecked(grandChild.id, checked, false)
        })
      }
    })
...
const updateParentNodeStatus = () => {
  ...
  const allNodes = getAllNodes(permissionList.value)
  
  // 检查每个父节点的子节点是否全部选中
  permissionList.value.forEach(parent => {
    if (parent.children && parent.children.length > 0) {
      const allChildrenSelected = parent.children.every(child => {
        return checkedNodes.some(node => node.id === child.id)
      })
      
      // 更新父节点的选中状态
      permissionTree.value.setChecked(parent.id, allChildrenSelected, false)
    }
  })
  
  // 检查是否所有节点都被选中，如果是，则选中"全选"
  // 获取除全选节点外的所有节点
  const allNodesExceptRoot = allNodes.filter(node => node.id !== 0)
  
  // 检查是否所有节点都被选中
  const allSelected = allNodesExceptRoot.every(node => {
    return checkedNodes.some(checkedNode => checkedNode.id === node.id)
  })
  
  // 更新全选节点的状态
  permissionTree.value.setChecked(0, allSelected, false)
}
...
const savePermissions = () => {
  ...
  // 例如: saveRolePermissions(currentRole.value.id, checkedKeys)
...
/* 树形控件样式 */
.permission-content :deep(.el-tree) {
...
.permission-content :deep(.el-tree-node__content) {
  height: 36px;
}

.permission-content :deep(.el-tree-node__content:hover) {
  background-color: #f5f7fa;
}

.permission-content :deep(.el-checkbox__input.is-checked .el-checkbox__inner) {
