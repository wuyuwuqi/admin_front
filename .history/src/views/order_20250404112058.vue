      ...
      <el-card class="info-card order-info-card">
        <template #header>
          <div class="card-header">
            <span>订单信息</span>
            <el-tag :type="getStatusType(currentOrderDetail.status)">{{ getOrderStatusText(currentOrderDetail.status) }}</el-tag>
          </div>
        </template>
        <div class="info-grid">
          <div class="info-item">
            <span class="info-label">订单编号</span>
            <span class="info-value">{{ currentOrderDetail.id }}</span>
          </div>
          <div class="info-item">
            <span class="info-label">下单时间</span>
            <span class="info-value">{{ currentOrderDetail.createTime }}</span>
          </div>
          <div class="info-item">
            <span class="info-label">付款方式</span>
            <span class="info-value">{{ currentOrderDetail.paymentMethod || '在线支付' }}</span>
          </div>
          <div class="info-item">
            <span class="info-label">付款状态</span>
            <span class="info-value">{{ currentOrderDetail.paymentStatus }}</span>
          </div>
          <div class="info-item">
            <span class="info-label">订单金额</span>
            <span class="info-value">¥ {{ currentOrderDetail.totalPrice }}</span>
          </div>
        </div>
      </el-card>
      ...
      <el-card class="info-card house-info-card">
        ...
        <div class="info-grid">
          <div class="info-item">
            ...
            <span class="info-value">{{ currentOrderDetail.accommodationName }}</span>
          </div>
          <div class="info-item">
            <span class="info-label">详细地址</span>
            <span class="info-value">{{ currentOrderDetail.accommodationAddress || '暂无地址信息' }}</span>
          </div>
          <div class="info-item">
            <span class="info-label">入住日期</span>
            <span class="info-value">{{ currentOrderDetail.startDate }}</span>
          </div>
          <div class="info-item">
            <span class="info-label">退房日期</span>
            <span class="info-value">{{ currentOrderDetail.endDate }}</span>
          </div>
          <div class="info-item">
            <span class="info-label">入住天数</span>
            <span class="info-value">{{ currentOrderDetail.checkDays }}天</span>
          </div>
        </div>
      </el-card>
      
      <!-- 房间信息区块 -->
      <el-card class="info-card room-info-card" v-if="currentOrderDetail.checkInfo">
        <template #header>
          <div class="card-header">
            <span>房间信息</span>
          </div>
        ...
        <div class="info-grid">
          <div class="info-item" v-if="currentOrderDetail.checkInfo.checkNumber">
            <span class="info-label">房间数量</span>
            <span class="info-value">{{ currentOrderDetail.checkInfo.checkNumber }}</span>
          </div>
          <div class="info-item" v-if="currentOrderDetail.checkInfo.checkType">
            <span class="info-label">房间类型</span>
            <span class="info-value">{{ currentOrderDetail.checkInfo.checkType }}</span>
          </div>
          <div class="info-item" v-if="currentOrderDetail.checkInfo.checkFeatures">
            <span class="info-label">房间特点</span>
            <span class="info-value">{{ currentOrderDetail.checkInfo.checkFeatures }}</span>
          </div>
          <div class="info-item" v-if="currentOrderDetail.checkInfo.checkFloor">
            <span class="info-label">所在楼层</span>
            <span class="info-value">{{ currentOrderDetail.checkInfo.checkFloor }}</span>
          </div>
          <div class="info-item" v-if="currentOrderDetail.checkInfo.checkArea">
            <span class="info-label">房间面积</span>
            <span class="info-value">{{ currentOrderDetail.checkInfo.checkArea }}</span>
          </div>
        </div>
      </el-card>
      ...
      <el-card class="info-card customer-info-card">
        ...
        <div class="info-grid">
          <div class="info-item">
            ...
            <span class="info-value">{{ getGuestName(currentOrderDetail) }}</span>
          </div>
          <div class="info-item">
            <span class="info-label">联系电话</span>
            <span class="info-value">{{ getGuestPhone(currentOrderDetail) }}</span>
          </div>
          <div class="info-item">
            <span class="info-label">入住人数</span>
            <span class="info-value">{{ currentOrderDetail.guestsStaying ? currentOrderDetail.guestsStaying.length : 0 }}人</span>
          </div>
        </div>
      </el-card>
      
      <!-- 底部操作按钮 -->
      <div class="action-buttons dialog-buttons">
        <el-button @click="orderDetailDialogVisible = false">关闭</el-button>
        <el-button 
          type="primary" 
          v-if="currentOrderDetail.status === 'unpaid'"
          @click="confirmOrder(currentOrderDetail.id)"
        >确认支付</el-button>
      </div>
    </div>
  </el-dialog>
...
// 获取入住人姓名
const getGuestName = (orderData) => {
  if (orderData.guestsStaying && orderData.guestsStaying.length > 0) {
    return orderData.guestsStaying[0].name
  }
  return '未知用户'
}

// 获取入住人电话
const getGuestPhone = (orderData) => {
  if (orderData.guestsStaying && orderData.guestsStaying.length > 0) {
    return orderData.guestsStaying[0].phone
  }
  return '无'
...
const viewOrderDetail = async (orderId) => {
  ...
  try {
    ...
    const config = {
      method: 'get',
      url: `http://localhost:8080/booking/bookingByOrderId?orderId=${orderId}`,
      headers: {
        'Authorization': token,
        'Username': username
      }
    ...
    if (response.data) {
      ...
      currentOrderDetail.value = {
        ...
        checkInfo: response.data.checkInfo || {
          checkNumber: '',
          checkType: '',
          checkFeatures: '',
          checkFloor: '',
        ...
        userMessage: response.data.userMessage || '无留言'
      }
      console.log('订单详情数据:', currentOrderDetail.value)
    } else {
      ElMessage.warning('未获取到订单详情数据')
    }
  } catch (error) {
...
watch(
  ...
  ([newPath, newOrderStatus]) => {
    ...
    if (newOrderStatus) {
      ...
      // 检查是否是有效的订单状态
      if (statusMap[convertRouteStatusToApiStatus(statusSegment)]) {
