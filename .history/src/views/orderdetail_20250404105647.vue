<template>
  <div class="order-detail-page">
    ...
    <div v-else>
        ...
        <div class="house-info">
          ...
          <div class="info-grid">
            ...
            <div class="info-item">
              ...
              <span class="info-value">{{ order.accommodationAddress || order.houseAddress || '暂无地址信息' }}</span>
            </div>
            <div class="info-item">
            ...
            <div class="info-item">
              ...
              <span class="info-value">{{ order.checkDays || '1' }}天</span>
            </div>
            <div class="info-item">
      ...
      <!-- 房间信息区块 -->
      <el-card class="info-card room-info-card" v-if="order.checkInfo">
        <template #header>
          <div class="card-header">
            <span>房间信息</span>
          </div>
        ...
        <div class="info-grid">
          <div class="info-item" v-if="order.checkInfo.checkNumber">
            <span class="info-label">房间数量</span>
            <span class="info-value">{{ order.checkInfo.checkNumber }}</span>
          </div>
          <div class="info-item" v-if="order.checkInfo.checkType">
            <span class="info-label">房间类型</span>
            <span class="info-value">{{ order.checkInfo.checkType }}</span>
          </div>
          <div class="info-item" v-if="order.checkInfo.checkFeatures">
            <span class="info-label">房间特点</span>
            <span class="info-value">{{ order.checkInfo.checkFeatures }}</span>
          </div>
          <div class="info-item" v-if="order.checkInfo.checkFloor">
            <span class="info-label">所在楼层</span>
            <span class="info-value">{{ order.checkInfo.checkFloor }}</span>
          </div>
          <div class="info-item" v-if="order.checkInfo.checkArea">
            <span class="info-label">房间面积</span>
            <span class="info-value">{{ order.checkInfo.checkArea }}</span>
          </div>
          <div class="info-item" v-if="order.checkInfo.checkRequirements">
            <span class="info-label">特殊要求</span>
            <span class="info-value">{{ order.checkInfo.checkRequirements }}</span>
          </div>
        </div>
      </el-card>
  ...
  try {
    ...
    if (response.data) {
      ...
      order.value = {
        ...
        checkInfo: orderData.checkInfo || {},
        checkDays: orderData.checkDays || '',
        logs: []
      }
      
      // 如果有checkInfo信息，添加到订单信息中
      if (orderData.checkInfo) {
        order.value.checkInfo = orderData.checkInfo
      }
      
      // 如果有checkDays信息，添加到订单信息中
      if (orderData.checkDays) {
        order.value.checkDays = orderData.checkDays
      }
      }
...
const deleteOrder = async () => {
  try {
    ...
    const config = {
      method: 'delete',
      url: `/booking/deleteBooking?orderId=${orderId.value}&landlordId=${userInfo.id}`,
      headers: {
        'Authorization': token,
        'Username': userInfo.username
      }
