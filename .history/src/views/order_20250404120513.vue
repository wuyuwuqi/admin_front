  ...
  try {
    ...
    if (filterStatus.value) {
      ...
      if (apiStatus) {
        ...
        console.log('请求URL状态参数:', apiStatus) // 记录实际使用的状态参数
      }
    }
    
    console.log('完整请求URL:', url) // 记录完整请求URL
...
const fetchOrderDetail = async () => {
  ...
  try {
    ...
    const config = {
      method: 'get',
      url: `http://localhost:8080/booking/bookingByOrderId?orderId=${currentOrderId.value}`,
      headers: {
        'Authorization': token,
...
const convertRouteStatusToApiStatus = (routeStatus) => {
  // 路由状态和API状态的映射关系
  const statusMapping = {
    'pending-payment': 'pending', // 修改为后端实际接受的状态值
    'in-progress': 'ongoing',
  ...
  console.log('状态转换:', routeStatus, '->', statusMapping[routeStatus] || routeStatus)
