...
const handleLogin = async () => {
  ...
  try {
    ...
    if (response.data && response.data.code === '200') {
      // 登录成功，使用LandlordStorage类保存登录数据
      LandlordStorage.saveLoginData({
        landlord: response.data.userInfo,
      ...
      await nextTick()
      // 直接使用window.location.href进行跳转，避免可能的路由问题
      console.log('准备跳转到首页')
      // 确保localStorage中的认证状态已正确设置
      console.log('认证状态:', LandlordStorage.isAuthenticated())
      // 强制刷新页面以确保路由守卫重新评估认证状态
