    ...
    <el-card class="house-list-card">
      <el-table :data="filteredHouses" style="width: 100%" border stripe>
        ...
        <el-table-column label="操作" width="150" fixed="right">
          <template #default="scope">
            <el-button size="small" @click="handleEdit(scope.row.hotelId)">编辑</el-button>
...
const handleEdit = async (id) => {
  ...
  try {
    ...
    const response = await axios({
      method: 'get',
      url: `/api/hotel/byHotelId?hotelId=${id}&landlordId=${landlordId}`,
      headers: {}
    })
...
const saveHouse = async () => {
  try {
    ...
    if (isEdit.value) {
      ...
      const landlordId = requestData.landlordId
      ...
      const response = await axios({
        method: 'post',
        url: `/api/hotel/updateHotel?landlordId=${landlordId}&hotelId=${hotelId}`,
        headers: { 
      ...
      // 根据后端响应格式进行判断
      if (response.data.code === 200 || response.data.statusCode === 200) {
