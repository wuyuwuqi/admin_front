...
const handleLogin = async () => {
  ...
  try {
    console.log('登录请求参数:', {
      ...
      passwordHash: loginForm.value.password.substring(0, 3) + '***' // 安全起见只打印密码前几位
    })
    
    // 调用登录API
    const response = await axios.post('http://wangjunxishi.ct.ws:8080/landlord/login', null, {
      params: {
        phoneNumber: loginForm.value.username,
    ...
    if (response.data && response.data.code === 200) {
      ...
      ElMessage.error({
        message: response.data.message || '登录失败，请检查账户和密码',
        duration: 5000
      })
    }
  } catch (error) {
    ...
    if (error.response) {
      ...
      ElMessage.error(`登录失败: ${error.response.data?.message || error.message || '服务器错误'}`)
    } else if (error.request) {
      console.error('未收到响应:', error.request)
      ElMessage.error('登录失败: 服务器未响应，请检查网络连接')
    } else {
